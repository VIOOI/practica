			...
			<div className="w-full">
				<div className="w-full flex justify-between mb-2">
					<h2 className='text-gray-600 dark:text-white font-bold'>
						<span className="dark:bg-white bg-black inline-block px-2 rounded-md text-white dark:text-black mr-2 font-extrabold">{ type ?? 'Документ' }</span>
				...
				? (
					<div className="text-zinc-500 dark:text-white mt-3">
						<span className="text-xs pr-5">{ docDate ?? '00.00.0000' }</span>
						<span className=" inline-block rounded-md text-white">{ auth ?? 'Автор' }</span>
					</div>
				) : null
				}
			</div>
